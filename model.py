...
class Model:
    ...
    def online_compute_winning_region(self, obs_initial_node, AccStates, Avalid_states, observation_successor_map, H, ACP, dfa = None):
        #--------------ONLINE-------------------------
...
def create_scenario_obstacle(minX, minY, maxX, maxY, 
    ...
    dx, dy = 1, 1
    # dx, dy = 0.5, 0.5
    for i in np.arange(minX, maxX + 1, dx):
        for j in np.arange(minY, maxY + 1, dy):
            node = (i, j)
    ...
    for fnode in robot_nodes: 
        ...
        for action_index, action in enumerate(actions):
            ...
            if action_index == 0:
                nxt_states = {(i, j + 2 * dy):slippery_prob, (i, j + 1 * dy): (1-slippery_prob)}
                # nxt_states = {(i, j + 2):slippery_prob, (i, j + 1): (1-slippery_prob)}
            elif action_index == 1:
                nxt_states = {(i, j - 2 * dy):slippery_prob, (i, j - 1 * dy): (1-slippery_prob)}
                # nxt_states = {(i, j - 2):slippery_prob, (i, j - 1): (1-slippery_prob)}
            elif action_index == 2:
                nxt_states = {(i + 2 * dx, j):slippery_prob, (i + 1 * dx, j): (1-slippery_prob)}
    ...
    for fx, fy in motion_mdp.nodes():
        if (fx, fy) in obstacles:
            ...
            # state_observation_map[(fx, fy)] = (10000 + fx//2, 10000 + fy//2)
